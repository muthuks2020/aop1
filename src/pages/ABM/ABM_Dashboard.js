...
function ABMDashboard() {
  ...
  // teamMembers: all direct reports loaded from /abm/team-members (not derived from submissions)
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      // Load ALL direct-report team members (independent of submissions)
      const members = await ABMApiService.getTeamMembers();
  ...
  const filteredSubmissions = useMemo(() => {
    ...
    if (tbmFilter !== 'all') filtered = filtered.filter(s => s.tbmId === tbmFilter || s.employeeCode === tbmFilter);
      ...
      <main className="main excel-main">
            ...
            <div className="abm-approval-filters">
              <div className="abm-filter-group">
                ...
                <select value={tbmFilter} onChange={(e) => setTbmFilter(e.target.value)}>
                  ...
                  {teamMembers.map(tbm => (
                    <option key={tbm.employeeCode} value={tbm.employeeCode}>
                      {tbm.fullName} — {tbm.territory}
                    </option>
                  ))}
                </select>
              </div>
        ...
        {activeTab === 'yearlyTargets' && (
          <TeamYearlyTargets
            role="ABM"
            fiscalYear="2026-27"
            teamMembers={teamMembers.map(m => ({ id: m.employeeCode, name: m.fullName, territory: m.territory }))}
            showToast={showToast}
