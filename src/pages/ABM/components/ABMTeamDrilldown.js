 ...
 * 
 * PART 3 — Item 8: Added LY Ahv, CY Ahv, CY Target columns to drilldown cards
    ...
    tbm.salesReps.forEach(rep => {
      rep.products?.forEach(p => {
        MONTHS.forEach(m => {
          ...
          // PART 3 — Item 8: achievement fields (backend may not have these yet — default to 0)
            ...
            <div key={tbm.id} className={`abm-dd-tbm-card ${isExpanded ? 'expanded' : ''}`}>
              ...
              <div className="abm-dd-tbm-header" onClick={() => toggleTBM(tbm.id)}>
                ...
                <div className="abm-dd-tbm-stats">
                  ...
                  <div className="abm-dd-mini-stat">
                    <span className="abm-dd-mini-label">CY Target</span>
                    <span className="abm-dd-mini-value">₹{Utils.formatCompact(totals.totalCyRev)}</span>
                  </div>
                  {/* PART 3 — Item 8: LY Ahv and CY Ahv */}
                  <div className="abm-dd-mini-stat">
                    <span className="abm-dd-mini-label">LY Ahv</span>
                    <span className="abm-dd-mini-value">
                      {totals.totalLyAchRev > 0 ? `₹${Utils.formatCompact(totals.totalLyAchRev)}` : '—'}
                    </span>
                  </div>
                  <div className="abm-dd-mini-stat">
                    <span className="abm-dd-mini-label">CY Ahv</span>
                    <span className="abm-dd-mini-value">
                      {totals.totalCyAchRev > 0 ? `₹${Utils.formatCompact(totals.totalCyAchRev)}` : '—'}
                    </span>
                  </div>
                  <div className="abm-dd-mini-stat">
